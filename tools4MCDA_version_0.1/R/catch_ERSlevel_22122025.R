#' Convert the estimated landings weight or value for SSF expressed in tonnes or euro using Fishing Pressure Index and Landings Per Unit of Effort as a proxy in a grid cell of 0.5*0.5 decimal degrees (csquare) in order to inform Table H (version 2 - detailed).
#'
#' Convert in csquare (0.5 * 0.5 dd) the combined outcome resulted based on FPI (estimated by MCDA), the LPUE and the Fisheries Dependent Information (FDI) data (Table A in the data call) to perform spatial estimations of landings weight or value and inform the Table H (version 2 - detailed).
#' 
...
#' @param parameter Name of the field that contains the landings weight or value in the data table using "".
#'  
...
#'                              metier="metier",metier_7="metier_7",mesh_size_range="mesh_size_range",parameter = "totwghtlandg",LON="LON",LAT="LAT")
#'
...
#' @export
catch_ERSlevel2<-function (data_FPI, ERS_poly, LON="LON",LAT="LAT",parameter= "LW") 
{
  library(sf)
  FPI <- data_FPI
  FPI$LON <- FPI[[LON]]
  ...
  FPI_ERS <- FPI %>%
    group_by(
      ...
      fishing_tech, gear_type, target_assemblage,
      metier_7, metier, mesh_size_range,
      Sub.region, csq_x, csq_y, cscode, CELLCODE, species,
      .drop = FALSE
    ) %>%
    summarise(parameter = sum(parameter, na.rm = TRUE), .groups = "drop")

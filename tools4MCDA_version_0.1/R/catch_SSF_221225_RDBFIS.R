#' Estimate landing weight or value using Fishing effort and landings per unit effort as a proxy (version 2 - detailed).
#'
...
#' @param metier_7 The field in FPI table that contains the metier_7 using "".
#' @param mesh_size_range The field in FPI table that contains the mesh_size_range using "".
...
#'                              FE="FPI", LPUE="LPUE", species="species", table_a=table_a,gear="gear_type",year="year",
#'                              quarter="quarter", vessel_length_cat="vessel_length",fishing_tech ="fishing_tech",target_assemblage="target_assemblage",
#'                              metier="metier",metier_7="metier_7",mesh_size_range="mesh_size_range",parameter = "totvallandg",LON="LON",LAT="LAT")
#' @export
landings_SSF2 <- function(data, Sub.region , Country , FE , LPUE, species, table_a, gear, year,quarter,
                         metier, metier_7, mesh_size_range, fishing_tech, target_assemblage,vessel_length_cat, parameter= "totwghtlandg",LON, LAT) {
 ...
 table_a$parameter<- table_a[[parameter]]
  ...
  FPI2<-unite(FPI, col='joinFDI', c('Country','year','quarter', 'vessel_length_cat','fishing_tech','gear','target_assemblage','metier','metier_7','mesh_size_range','Sub.region', 
                                      'species',), sep='_',remove = FALSE)
  table_a2<-unite(table_a, col='joinFDI', c('country','year','quarter','vessel_length','fishing_tech','gear_type','target_assemblage','metier','metier_7','mesh_size_range','sub_region', 
                                             'species'), sep='_')

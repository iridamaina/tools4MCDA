...
#' @param year year of species distribution
#'  
...
#'library(tools4MCDA)
#'data(sample_FPIq)
#'data_with_LPUE<-add_LPUE_year(data_FPI=sample_FPIq,spec=c("HKE","MUT"),folder="_Your_folder_",name=c("hke.csv","mut.csv"), field=c("grid_code","grid_code"), LON="POINT_X", LAT="POINT_Y", sep="," , year=2019)
#' @export
add_LPUE_year<-function (data_FPI=sample_FPIq,spec=c("HKE","MUT"),
                    folder="",
                    name=c("hke.txt","mut.txt"), field=c("grid_code","grid_code"), LON="POINT_X", LAT="POINT_Y", sep=",",year )
{
  s_r1<-NULL
  sample_spq<-NULL
  sp_sample<-NULL
...
for(y in year){
  ...
  s_data<-s_data[s_data$year==y,]
s_r1 <- rasterFromXYZ(s_data[, c("LON", "LAT", "field")])
crs(s_r1) <- "+proj=longlat +datum=WGS84 +no_defs +ellps=WGS84 +towgs84=0,0,0"  
fpi_agreegate<-aggregate(FPI~LON+LAT+year,FUN=mean, data=data_FPI)
fpi_agreegate<-fpi_agreegate[fpi_agreegate$year==y,]
...
sp_sample$year<-y
sp_sample<-unite(sp_sample, col='key_sp', c('LON','LAT','year'), sep='_', remove=FALSE)
data_FPI<-unite(data_FPI, col='key_sp', c('LON','LAT','year'), sep='_', remove=FALSE)
data_FPI2<-data_FPI[data_FPI$year==y,]
sp_sample_v2<-merge(x = sp_sample[,c("key_sp","species","LPUE")], y =data_FPI2 , by = "key_sp", all.y = TRUE)
#sp_sample_v2$year<-y

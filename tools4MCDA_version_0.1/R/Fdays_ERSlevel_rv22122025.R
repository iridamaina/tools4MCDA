#' Convert the estimated fishing effort for SSF expressed in days at sea using Fishing Pressure Index as a proxy in a grid cell of 0.5*0.5 decimal degrees (csquare) in order to inform Table I (version 2 - detailed).
#'
#' Convert in csquare (0.5 * 0.5 dd) the combined Fishing Pressure Index (FPI- estimated by MCDA) with Fisheries Dependent Information (FDI) data (Table G in the data call) to perform spatial estimations of fishing effort (days at sea) and inform the Table I (version 2 - detailed).
#' 
...
#' @export
Fdays_ERSlevel2<-function (data_FPI, ERS_poly, LON="LON",LAT="LAT",parameter= "FDays") 
{
  library(sf)
  FPI <- data_FPI
  FPI$LON <- FPI[[LON]]
  ...
  #FPI_ERS<-aggregate(data=FPI,cbind(parameter)~ Country + year+quarter+vessel_length+fishing_tech+gear_type+target_assemblage+
    #                   metier_7+metier+mesh_size_range+Sub.region+csq_x+csq_y+cscode+CELLCODE, FUN=sum)
  library(dplyr)
  FPI_ERS <- FPI %>%
    group_by(
      ...
      Sub.region, csq_x, csq_y, cscode, CELLCODE,
      .drop = FALSE
    ) %>%
